...
public class ChatCooldownFix
{
		...
		(new Thread() {
			public void run() {
				while (true) {
			        ...
			        if (net.minecraftforge.client.ClientCommandHandler.instance.executeCommand(Minecraft.getMinecraft().thePlayer, queue.get(0)) == 0)
    ...
    @SubscribeEvent
    public void onGuiOpen(GuiOpenEvent event) throws NoSuchFieldException, SecurityException, IllegalArgumentException, IllegalAccessException {
    	String name = "";
    	try {
    	...
    	if (name == "net.minecraft.client.gui.GuiChat") {
    		String defaultInputFieldText = ReflectionHelper.getPrivateValue(GuiChat.class, (GuiChat)event.gui, "defaultInputFieldText", "field_146409_v");
